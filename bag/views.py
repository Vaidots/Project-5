    ...
    if metal:
        if item_id in list(bag.keys()):
            if metal in bag[item_id]['items_by_metal'].keys():
                ...
                messages.success(request, f'Updated {metal.upper()} {product.name} quantity to {bag[item_id]["items_by_metal"][metal]}')

            else:
                bag[item_id]['items_by_metal'][metal] = quantity
                messages.success(request, f'Added {metal.upper()} {product.name} to your bag')
    ...
    if metal:
        if quantity > 0:
            ...
            if not bag[item_id]['items_by_metal']:
                bag.pop(item_id)
                messages.success(request, f'Removed  {metal.upper()} {product.name} from your bag')
    else:
        if quantity > 0:
            bag[item_id] = quantity
            messages.success(request, f'Updated {product.name} quantity to {bag[item_id]}')
